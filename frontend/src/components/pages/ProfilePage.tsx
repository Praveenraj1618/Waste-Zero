...
export function ProfilePage({ userRole, userName, userEmail }: ProfilePageProps) {
  ...
  const [bio, setBio] = useState(
    userRole === 'volunteer' 
      ? "Passionate about environmental sustainability and waste reduction. Dedicated to making a positive impact in my community."
      : "Environmental NGO committed to creating sustainable waste management solutions for communities."
  );
  ...
  const [skills, setSkills] = useState<string[]>(
    userRole === 'volunteer' 
      ? ["Recycling", "Composting", "Community Outreach"] 
      : []
  );
  const [orgName, setOrgName] = useState(
    userRole === 'ngo' ? "Green Earth Foundation" : ""
  );
  const [contactPerson, setContactPerson] = useState(
    userRole === 'ngo' ? "Jane Director" : ""
  );
  const [serviceAreas, setServiceAreas] = useState(
    userRole === 'ngo' ? "Downtown, Suburbs, Industrial District" : ""
  );
